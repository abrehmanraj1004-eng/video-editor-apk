...
if st.button("⚡ START AI VIDEO EDITING"):
    if not input_video_path and not yt_url:
        ...
        try:
            # Fetch YouTube title & download stream
            ...
            status_text.success("🎉 Video Processed Successfully! Downloading to your device...")

            # Clean safe filename from YouTube title
            safe_name = re.sub(r'[^\w\s\.-]', '_', video_title).strip()
            safe_name = re.sub(r'\s+', ' ', safe_name)[:90]
            if not safe_name:
                safe_name = "Edited_Video"
            if not safe_name.lower().endswith(".mp4"):
                download_filename = f"{safe_name}_61s.mp4"
            else:
            ...
            # Automatic Download to Phone / PC with YouTube Title
            with open(output_path, "rb") as file:
                video_bytes = file.read()
                b64_video = base64.b64encode(video_bytes).decode()
                
                # Automatic download trigger via hidden link
                components.html(f"""
                <a id="auto_dl_btn" href="data:video/mp4;base64,{b64_video}" download="{download_filename}" style="display:none;"></a>
                <script>
                    setTimeout(function() {{
                        var btn = document.getElementById('auto_dl_btn');
                        if (btn) {{
                            btn.click();
                        }}
                    }}, 500);
                ...
                st.download_button(
                    label=f"💾 Download Video: {download_filename}",
                    data=video_bytes,
